...
public class OutboxEventRelayer {
    // OutboxRepository'imizden verileri okuyacak,streamBridge'a
    // gönderecek,gönderirken de verileri okuması için mapper
    ...
    @Scheduled(fixedRate = 5000) // 5sn'de bir arkada çalıştılrılması sağlanır(spring boot otomatik tetikleyecek)
    public void publishPendingEvents() throws JsonProcessingException {
        System.out.println("publish pending events çalıştı");
        // eventlerin işlenmesi
        // gönderilmemiş ve fail olmamış eventleri repodan bul
        List<OutboxMessage> pendingEvents = outboxRepository.findByStatusOrderByCreatedAt(OutboxStatus.PENDING);
        // bu eventleri göndermeye çalış
        for (OutboxMessage pendingEvent : pendingEvents) {
            // eventi db'ye yazarken objectMapper'la stringe çevirmiştik,
            // burada tekrardan event'e çevirmemiz lazım ki gönderebilelim
            // deserialize
